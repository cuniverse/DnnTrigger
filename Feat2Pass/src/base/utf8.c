...
extern unsigned short ksc2uni_code[18366];
extern unsigned short uni2ksc_code[11172];
...
PowerASR_Base_encode_UTF8(const unsigned char *instr,
						  ...
						  int *outlen)
{ 
	unsigned char midstr[1024]; 
	int i, code;
	int pmidstr = 0;
	
	for (i=0; i < szinstr; i++) {
		if ((unsigned) instr[i] < 128) { 
			midstr[pmidstr++] =  instr[i];
		}
		else {
			code = ((unsigned char)instr[i])*256 + (unsigned char)instr[i+1] - 0x8141;
			if (code < 0 || code > 18365) {
				return -1;
			}
			else {
				if (ksc2uni_code[code]>=0x800 && ksc2uni_code[code]<=0xFFFF) {
					midstr[pmidstr++] =(unsigned char)(((ksc2uni_code[code]&0xf000)>>12)|0xE0);
					midstr[pmidstr++] =(unsigned char)(((ksc2uni_code[code]&0xfc0 )>>6 )|0x80);
					midstr[pmidstr++] =(unsigned char)(( ksc2uni_code[code]&0x3f       )|0x80);
				} 
				else if (ksc2uni_code[code]>=0x80) {
					midstr[pmidstr++] =(unsigned char)(((ksc2uni_code[code]&0x7c0)>>6  )|0x80);
					midstr[pmidstr++] =(unsigned char)(( ksc2uni_code[code]&0x3f       )|0x80);
				}
				else if (ksc2uni_code[code]>=0x1) {
					midstr[pmidstr++] =(unsigned char)(ksc2uni_code[code]);
				}
				else {  /* if 0x0 */
					midstr[pmidstr++] =(unsigned char) 0xc0;
					midstr[pmidstr++] =(unsigned char) 0xa0;
				}
			}
			i++;
		}
	...
	*outlen = pmidstr;
	*outstr = (unsigned char *) malloc(pmidstr+1);
	memset( *outstr, 0x00, pmidstr+1);
	for (i=0; i<pmidstr; i++) (*outstr)[i] = midstr[i];
...
//	midstr[0] = 0;
//	memset(midstr, 0, 1024);
	
	for (i=0; i < inlen; i++) {
		if (!(instr[i] & 0x80)) {  /* \u0001 - \u007f */
			outstr[pmidstr++] =  instr[i];
		}
		else if ((instr[i] & 0xE0) == 0xC0) { /* \u0080 - \u07ff */
			outstr[pmidstr++] = (unsigned char) ' ';
		}
		else if ((instr[i] & 0xF0) == 0xE0) { /* \u0800 - \uffff */
			if ((instr[i+1] & 0xC0) == 0x80 && (instr[i+2] & 0xC0) == 0x80) {
//				code = (instr[i] & 0x0F) * 4096;  /* 2**12 */
//				code += (instr[i+1] & 0x3F) * 64;   /* 2**6 */
				code = 0x0000;
				code = (instr[i] & 0x0F) << 12;  /* 2**12 */
				code += ((instr[i+1] & 0x3F) << 6);   /* 2**6 */
				code += (instr[i+2] & 0x3F);
				
				if (code < 0xAC00 || code > 0xD7A3) {
					outstr[pmidstr++] = (unsigned char) ' ';
//					TRACE("code < 0xAC00 || code > 0xD7A3\n");
					i+=2;
				}
				else {
//					outstr[pmidstr++] = (unsigned char) (uni2ksc_code[code-0xAC00] / 256);
					outstr[pmidstr++] = (unsigned char) (uni2ksc_code[code-0xAC00] >> 8);
					outstr[pmidstr++] = (unsigned char) (uni2ksc_code[code-0xAC00] % 256);
					i+=2;
				}
			}
...
//	midstr[pmidstr] = 0;
//	*outstr = (char *) malloc(pmidstr+1);
//	memset( *outstr, 0x00, pmidstr+1);

//	strcpy(*outstr, (char *)midstr);
//	*szoutstr = pmidstr;
	outstr[pmidstr] = 0;
	return pmidstr;
}
...
PowerASR_Base_is_UTF8(unsigned char* str)
{
	...
	for(i = 0; i < len;i++) {
		...
		if ( c >= 0xc0 && c <= 0xdf ) {
			if (i + 1 >= len) return 0;
			c1 = str[++i];
			if ( c1 >= 0x80 && c1 <= 0xbf) continue; else return 0;
		}
			...
			c2 = str[++i];
			if (    c1 >= 0x80 && c1 <= 0xbf &&
				c2 >= 0x80 && c2 <= 0xbf        ) continue; else return 0;
		}
			...
			if (    c1 >= 0x80 && c1 <= 0xbf &&
				c2 >= 0x80 && c2 <= 0xbf &&
				c3 >= 0x80 && c3 <= 0xbf        ) continue; else return 0;
		}
			...
			if (    c1 >= 0x80 && c1 <= 0xbf &&
				c2 >= 0x80 && c2 <= 0xbf &&
				c3 >= 0x80 && c3 <= 0xbf &&
				c4 >= 0x80 && c4 <= 0xbf        ) continue; else return 0;
		}
			...
			if (    c1 >= 0x80 && c1 <= 0xbf &&
				...
				c3 >= 0x80 && c3 <= 0xbf &&
				c4 >= 0x80 && c4 <= 0xbf &&
				c5 >= 0x80 && c5 <= 0xbf        ) continue; else return 0;
		}
		else
			return 0;
	}
...
// 주어진 텍스트가 UTF-8 코드인지 판별
HCILAB_PUBLIC HCI_BASE_API hci_int32
PowerASR_Base_is_UTF8(unsigned char* instr)
{ 
	int i;
	unsigned short code;
	int pmidstr = 0;
	int inlen = strlen(instr);
	hci_int32 isUTF8 = TRUE;
	hci_int32 isHangul = FALSE;

	for (i=0; i < inlen && isUTF8 ; i++) {
		if (!(instr[i] & 0x80)) {  /* \u0001 - \u007f */
			;
		}
		else if ((instr[i] & 0xE0) == 0xC0) { /* \u0080 - \u07ff */
							...
							int cbSize)
{
	...
	if ( !szUniStr ) return szAnsi;
	
	nLen = wcstombs(NULL, szUniStr, 0);
	
	memset(szFront, 0, sizeof(szFront));
	memcpy(szFront, szUniStr, 100);
	if ( nLen == 0 ) return szAnsi;
	
	szAnsi = (char *) hci_calloc(nLen+2, 1);
	wcstombs(szAnsi, szUniStr, nLen+1);
...
/*
	...
	nLen = WideCharToMultiByte(CP_ACP, 0, szUniStr, cbSize, NULL, 0, NULL, NULL);

	if ( nLen == 0 ) return szAnsi;

	szAnsi = (char *) hci_calloc(nLen+2, 1);

	WideCharToMultiByte(CP_ACP, 0, szUniStr, cbSize, szAnsi, nLen, NULL, NULL);
...
PowerASR_Base_UnicodeBigEndianToUnicode(wchar16_t* scrString, long cbsize)
{
	char *pStr = (char*)scrString;
	char t;
	long i = 0;

	if ( !scrString || cbsize <= 0 ) return;
...
char *UTF8toKSC5601(char *szUTF8)
{
	int		nSize, nRe;
	char	*szTemp;
	
	unsigned char *vtext = (unsigned char*)malloc(strlen(szUTF8) + 1);

	memset(vtext, 0, strlen(szUTF8) + 1);
/*
	BOOL isKorean = ParseVXML(szUTF8, vtext);
	if(FALSE == isKorean) {
		return NULL;
	}
*/
	strcpy(vtext, szUTF8);
/*
	OutputDebugString("UTF8toKSC5601() : Korean UTF Text => ");
	OutputDebugString((char *)vtext);
*/
	nSize = (strlen((const char *)vtext) + 1) * 2;
	szTemp = (char *)calloc(nSize, 1);

	if(!PowerASR_Base_is_UTF8(vtext)) {
//		OutputDebugString("UTF8toKSC5601() : Not UTF8");
		return NULL;
	}

	nRe = PowerASR_Base_decode_UTF8(vtext, strlen((const char *)vtext), szTemp, nSize);
	if(nRe == -1) {
//		OutputDebugString("UTF8toKSC5601() : decode_UTF8 size is too long");
		return NULL;
	}
...
unsigned short ksc2uni_code[18366] = {
	0xAC02, 0xAC03, 0xAC05, 0xAC06, 0xAC0B, 0xAC0C, 0xAC0D, 0xAC0E,
		...
		unsigned short uni2ksc_code[11172] = {
