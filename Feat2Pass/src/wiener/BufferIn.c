 ...
 *
 *---------------------------------------------------------------------------*/
extern BufferIn *BufInAlloc (int BufSize)
{
  BufferIn *newBuf = calloc (1, sizeof (BufferIn));
  if (newBuf == NULL) return NULL;
  newBuf->data = calloc (BufSize, sizeof (newBuf->data[0]));
  if (newBuf->data == NULL) return BufInFree (newBuf);
  newBuf->size = BufSize;
  return newBuf;
}
...
extern int BufInPut (BufferIn *Buf, X_FLOAT32 *newData, int newDataSize)
{
  ...
  if (newDataSize > Buf->size) newDataSize = Buf->size;
  /*----------------
   * shift old data 
   *----------------*/
...
extern int BufInGetLast (BufferIn *Buf, X_FLOAT32 *dataOutBuf, int nbAskSamples)
{
  ...
  if (nbAskSamples > Buf->size) nbAskSamples = Buf->size;
