 ...
 *
 *---------------------------------------------------------------------------*/
void InitMelFBwindows (MelFB_Window *FirstWin, float StFreq, float SmplFreq, int FFTLength, int NumChannels, ETSI_BOOL normalize)
{
  int i, j, k;
  int *centrFreq;
  float freq;
  ...
  if (normalize)
    for (j=0; j < p1->Length; j++) p1->Data [j] /= normFBconst;
